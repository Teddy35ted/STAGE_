...
export class AppwriteMediaService {
  ...
  static readonly BUCKET_ID = 'la-a-la-media';
  ...
  static async uploadFile(options: MediaUploadOptions): Promise<MediaUploadResult> {
    try {
      ...
      // Upload du fichier avec le nom organisé
   ...
   */
  static async listFiles(category: MediaCategory, userId?: string, limit: number = 100): Promise<any[]> {
    try {
      // Note: Appwrite ne supporte pas le filtrage par nom de fichier/chemin
      // Cette fonction liste tous les fichiers et les filtre côté client
      const files = await storage.listFiles(this.BUCKET_ID, [], limit);
      
      // Filtrer par catégorie (basé sur les métadonnées ou convention de nommage)
      return files.files.filter(file => {
        // Ici on pourrait utiliser des tags ou métadonnées pour filtrer
        // Pour l'instant, on retourne tous les fichiers
        return true;
      });
    } catch (error) {
  ...
  static async getUsageStats(): Promise<Record<MediaCategory, { count: number; totalSize: number }>> {
    try {
      ...
      // Note: Sans métadonnées, difficile de catégoriser automatiquement
      // On pourrait utiliser des tags ou une base de données séparée pour tracker cela
...
// Types pour les composants (compatibilité avec l'ancienne version)
